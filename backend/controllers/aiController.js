        ...
        if (!suggestions.length) {
            suggestions = [
                ...
                },
                {
                    ...
                    category: "Learning",
                    icon: "📚",
                    reason: "Compounds into significance knowledge over weeks.",
                },
                {
...
export const morningMotivation = async (req, res) => {
    try {
        ...
        const logs = await HabitLog.find({
            userId: req.user._id,
            completedDate: {$gte: days[0], $lte: days[day.length - 1]},
        });
        ...
        const userMsg = `Todaay's habits and streaks:\n${ctx}\n\nDone today: ${done}/${total}. Write the morning message encouraging the user to complete today's remaining habits.`;
