...
export const chatCompletion = async ({ system, user, temperature = 0.7}) => {
    ...
    if (!c) {
        return {
            ok: false,
            content: 
                "AI features are disabled - set GEMINI_API_KEY in the backend .env to enable real AI responses. Meanwhile, here is a basic habit suggestion generated using the application's built-in logic.",
        };
    } 
    try {
...
export const SYSTEM_PROMPTS = {
    ...
    suggestion:
        "You are a helpful habit coach. Based on the user's goals, productive time, and past struggles, suggest exactly 3 personalised habits. Return valid JSON only with this shape: {\"suggestion\":[{\"name\":\"...\",\"description\":\"...\",\"frequency\":\"daily|weekly\",\"category\":\"Health|Fitness|Learning|Mindfulness|Productivity|Social|Finance|Creative|Other\",\"icon\":\"<emoji>\",\"reason\":\"...\"}]}. No prose outside JSON.",
    recovery:
        "You are a compasssionate habit recovery coach. The user broke a streak. Write a 3 day recovery plan tailored to this specific habit. Be warm but actionable. Use this structure: short empathethic opening (1-2 sentences), then Day 1 / Day 2 / Day 3 sections one concrete action each, then a closing line of encouragement. 150-220 words total.",
    chat:
        "You are a helpful habit analysis assistant. Answer the user's questions using ONLY the provided habit data as context. Be specific - cite actual habit names, days, percentage. Keep replies under 120 words. If the data is insufficient, say so briefly.",
    morning:
